...
class FamilyTreeApp {
  ...
  displayPersonInfo(person) {
    ...
    var formatRelationship = function(currentPerson, referencePerson, depth, relationType) {
      ...
      if (relationType === "cousin") {
        ...
        html += currentFullName + ' est <strong>' + (currentPerson.sex === "F" ? "une " : "un ") + cousinLabel + '</strong> ' +
          getPreposition(referenceFullName) + referenceFullName + '.<br>';
        html += referenceFullName + ' est <strong>' + (referencePerson.sex === "F" ? "une " : "un ") + refCousinLabel + '</strong> ' +
          getPreposition(currentFullName) + currentFullName + '.';
        html += '</div>';
        return html;
      }
  ...
  drawSiblings(g, nodes) {
    ...
    nodes.forEach(function(person) {
      ...
      // Vérifier si l'alternance gauche/droite causerait un chevauchement avec le conjoint
      var forceDirection = null;
      if (person.parent && person.parent.children) {
        var otherParent = person.parent.children.find(function(c) { return c !== person; });
        if (otherParent) {
          var spouseDir = otherParent.x > person.x ? 1 : -1;
          // Tester chaque fratrie côté conjoint avec le placement alterné
          siblings.some(function(_, index) {
      ...
      siblings.forEach(function(sibling, index) {
        var siblingX, siblingY = person.y;

        if (forceDirection !== null) {
          // Chevauchement détecté : toutes les fratries du côté opposé au conjoint
          siblingX = person.x + forceDirection * CONFIG.sibling.spacing * (index + 1);
        } else {
          // Pas de chevauchement : alterner gauche/droite
          var isLeft = index % 2 === 0;
          var offset = Math.floor(index / 2) + 1;
          var direction = isLeft ? -1 : 1;
          siblingX = person.x + direction * CONFIG.sibling.spacing * offset;
        }
