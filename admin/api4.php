...
class FamilyTreeAPI {
    ...
    private function getDocuments() {
        ...
        $stmt = $this->pdo->prepare("
            SELECT 
                ...
                m.date_mariage as date_mariage,
                NULL as file_name,
                NULL as file_path,
                NULL as file_size,
                NULL as description,
        ...
        $stmt = $this->pdo->prepare("
            SELECT 
                'note' as type,
                'naissance' as source,
                'Naissance' as source_label,
                doc_naissance as content,
                'Source naissance' as title,
                NULL as file_name,
                NULL as file_path,
                NULL as file_size,
                NULL as description,
                ...
                NULL as mariage_id
            FROM membres 
            WHERE id = :person_id AND doc_naissance IS NOT NULL AND doc_naissance != ''
        ");
        $stmt->execute(['person_id' => $personId]);
        $naissanceNotes = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Récupérer les sources de décès
        $stmt = $this->pdo->prepare("
            SELECT 
                'note' as type,
                'deces' as source,
                'Décès' as source_label,
                doc_deces as content,
                'Source décès' as title,
                NULL as file_name,
                NULL as file_path,
                NULL as file_size,
                NULL as description,
                ...
                NULL as mariage_id
            FROM membres 
            WHERE id = :person_id AND doc_deces IS NOT NULL AND doc_deces != ''
        ");
        $stmt->execute(['person_id' => $personId]);
        $decesNotes = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Fusionner toutes les données
        $allDocuments = array_merge($documents, $memberNotes, $mariageNotes, $naissanceNotes, $decesNotes);
    ...
    private function updateDocument() {
        ...
        if (strpos($documentId, 'temp_') === 0) {
            ...
            if ($source === 'membre' && $type === 'note') {
            ...
            } elseif ($source === 'naissance' && $type === 'note') {
                $content = $_POST['content'] ?? '';
                $stmt = $this->pdo->prepare("UPDATE membres SET doc_naissance = :doc_naissance WHERE id = :id");
                $stmt->execute([
                    'doc_naissance' => $content,
                    'id' => $personId
                ]);
                
                return [
                    'success' => true,
                    'message' => 'Source de naissance mise à jour'
                ];
            } elseif ($source === 'deces' && $type === 'note') {
                $content = $_POST['content'] ?? '';
                $stmt = $this->pdo->prepare("UPDATE membres SET doc_deces = :doc_deces WHERE id = :id");
                $stmt->execute([
                    'doc_deces' => $content,
                    'id' => $personId
                ]);
                ...
                return [
                    'success' => true,
                    'message' => 'Source de décès mise à jour'
                ];
            } elseif (strpos($source, 'mariage_') === 0 && $type === 'note') {
                $mariageId = str_replace('mariage_', '', $source);
                $content = $_POST['content'] ?? '';
